<?php


require_once "/var/www/lib/class/common/OAuth.php";

class NateOAuthMethod extends OAuthSignatureMethod_RSA_SHA1 {
	protected function fetch_public_cert() {
		return <<<CERT
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
CERT;
	}
}

function nateoauth() {

	if(!$_GET['oauth_signature']) return false;
	$r = OAuthRequest::from_request(null, null, $_GET+$_POST);
	$sm = new NateOAuthMethod();
	$sv = $sm->check_signature($r, null, null, $_GET['oauth_signature']);
	return $sv;
}

