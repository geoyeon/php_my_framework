...
class Template_
{
	...
	var $template_dir_arr  = array( '_template' );
	var $prefilter     = '';
	var $postfilter    = '';
	var $permission    = 0777;
	var $safe_mode     = false;
	...
	function print_($fid, $scope='', $sub=0)
	{
		...
		{
			if ( RUN_LEVEL > RELEASE )
			{
				$this->exit_('Error #2', 'template id <b>'.$fid.'</b> is not defined/'.$this->p_control."/".$this->p_action."/");
			}
			else
			{
			}
	...
	function _get_compile_path($rpath)
	{
		...
		$tpl_path = "";
		$valid_path = false;
		for( $i=0; $i<count($this->template_dir_arr); $i++ )
		{
			$tpl_path=$this->template_dir_arr[$i].'/'.($this->skin?$this->skin.'/':'').$rpath;
	...
	{
		trigger_error( 'Template_ '.$type.': '.$msg );
	}
